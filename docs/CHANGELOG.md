...
## 2026-06-05 — Signal polling overlap: зниження ризику пропуску повідомлень із запізненою доставкою

- `signal-bridge/server.cjs`: додано 5-хвилинний буфер повідомлень (`messageBuffer`). При кожному виклику `/v1/receive` нові повідомлення зберігаються з міткою `receivedAt` (wall-clock). Обробник `/messages` тепер використовує параметр `?since=T` — повертає всі буферні записи де `receivedAt >= T` (плюс щойно отримані з черги). Старі записи видаляються автоматично. До цього `?since=` ігнорувався (`_req`), а bridge просто спустошував чергу signal-cli.
- `index.cjs`: новий параметр `SIGNAL_POLL_OVERLAP_MS` (за замовч. `60000`). Кожен поллінг запитує `effectiveSince = signalLastPollTs - SIGNAL_POLL_OVERLAP_MS` — тобто вікно завжди перекриває останні 60 секунд. Дублікати знімає вже наявний `signalSeenMessageIds`.
- Контекст: повідомлення `152.8750 / 18:01:00` з групи Лісництво 1.0 (05.06.2026) не було переслано через запізнену доставку на bridge — `sinceTs` вже просунувся вперед. З цим фіксом такі повідомлення підхоплюватимуться при наступному поллінгу.
